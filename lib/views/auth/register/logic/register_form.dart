...
class RegisterForm {
  ...
  DateTime minDate = DateTime(DateTime.now().year - 100), maxDate = DateTime.now();
  ...
  bool get validDocumentNumber  => Validators.validString(username, maxLength: 20);
  bool get validPassword        => Validators.validPassword(password);
  bool get validPassword2       => Validators.validPassword(password2) && password2 == password;
  bool get validMobile          => Validators.validNumber(mobile);
  bool get validDate            => date != null && minDate.isBefore(date) && maxDate.isAfter(date);
  bool get validEmail           => Validators.validEmail(email);

  bool get validForm => this.validName && this.validLastname && this.validUsername && this.validDocumentNumber && this.validPassword && this.validPassword2 && this.validMobile && this.validDate && this.validEmail;
